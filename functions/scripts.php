...
function mosbanglasearch_enqueue_scripts() {
	global $mosbanglasearch_options;
	wp_enqueue_script( 'jquery' );	
	wp_register_style( 'google-font', 'https://fonts.googleapis.com/css2?family=Nanum+Gothic:wght@400;700;800&display=swap' );
	...
	if ($mosbanglasearch_options['basic-styling-stylesheet']) {
		wp_register_style( $mosbanglasearch_options['basic-styling-stylesheet'], get_template_directory_uri() .  '/css/'.$mosbanglasearch_options['basic-styling-stylesheet'].'.css', array('main.min'));
		wp_enqueue_style( $mosbanglasearch_options['basic-styling-stylesheet'] );		
	}

}
add_action( 'wp_enqueue_scripts', 'mosbanglasearch_enqueue_scripts' );
function mosbanglasearch_admin_enqueue_scripts(){
	wp_register_style( 'font-awesome.min', get_template_directory_uri() . '/fonts/font-awesome-4.7.0/css/font-awesome.min.css' );
...
add_action( 'admin_enqueue_scripts', 'mosbanglasearch_admin_enqueue_scripts' );
